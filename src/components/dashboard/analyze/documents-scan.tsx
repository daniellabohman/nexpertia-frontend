...
import { uploadAnalyzeDocument } from "@/lib/api";
...
export function DocumentsScan(): React.JSX.Element {
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  const [analyseResultat, setAnalyzeResult] = useState<RisikoVurdering | null>(null);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      try {
        const response = await uploadAnalyzeDocument(file);
  
        setAnalyzeResult({
          dokumentNavn: file.name,
          brud: response.missing_elements || [], // Forventet fra backend
        });
      } catch (error) {
  ...
  return (
    <Card sx={{ p: 3, textAlign: "center" }}>
      <Stack spacing={2}>
        ...
        {valgtDokument && (
          <Button variant="contained" startIcon={<DownloadIcon />} onClick={downloadDocument}>
